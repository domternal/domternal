...
const staticMessages = detailsMessages;

describe('German details language pack', () => {
  it('covers exactly the owned message and searchable catalogs with frozen values', () => {
    const definitions = Object.values(detailsMessages);
  ...
  it('requires explicit opt in and leaves other editor services in English', () => {
    ...
    expect(german.t(detailsMessages.insert)).toBe('Aufklappbarer Block');
    expect(english.resolve(detailsMessages.insert)).toEqual({
      text: 'Toggle block', language: 'en', source: 'default',
    });
    german.set({ locale: 'de' });
    expect(german.resolve(detailsMessages.insert)).toEqual({
      text: 'Toggle block', language: 'en', source: 'default',
    });
    english.destroy();
    german.destroy();
