...
const staticMessages = imageMessages;

describe('German image language pack', () => {
  it('covers exactly the owned message and searchable catalogs with frozen values', () => {
    const definitions = Object.values(imageMessages);
  ...
  it('requires explicit opt in and leaves other editor services in English', () => {
    ...
    expect(german.t(imageMessages.insert)).toBe('Bild');
    expect(english.resolve(imageMessages.insert)).toEqual({
      text: 'Image', language: 'en', source: 'default',
    });
    german.set({ locale: 'de' });
    expect(german.resolve(imageMessages.insert)).toEqual({
      text: 'Image', language: 'en', source: 'default',
    });
    english.destroy();
    german.destroy();
