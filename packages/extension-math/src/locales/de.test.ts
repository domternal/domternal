...
const staticMessages = mathMessages;

describe('German math language pack', () => {
  it('covers exactly the owned message and searchable catalogs with frozen values', () => {
    const definitions = Object.values(mathMessages);
  ...
  it('requires explicit opt in and leaves other editor services in English', () => {
    ...
    expect(german.t(mathMessages.inline)).toBe('Formel im Text');
    expect(english.resolve(mathMessages.inline)).toEqual({
      text: 'Inline equation', language: 'en', source: 'default',
    });
    german.set({ locale: 'de' });
    expect(german.resolve(mathMessages.inline)).toEqual({
      text: 'Inline equation', language: 'en', source: 'default',
    });
    english.destroy();
    german.destroy();
