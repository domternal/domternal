...
describe('Table addToolbarItems', () => {
  it('returns a single dropdown item', () => {
    const items = Table.config.addToolbarItems?.call(Table);
    expect(items).toHaveLength(1);
    expect(items?.[0]?.type).toBe('dropdown');
  });

  it('dropdown has correct metadata', () => {
    const items = Table.config.addToolbarItems?.call(Table);
    const dropdown = items?.[0];
    if (dropdown?.type === 'dropdown') {
      expect(dropdown.name).toBe('table');
      expect(dropdown.icon).toBe('table');
      expect(dropdown.label).toBe('Table');
      expect(dropdown.group).toBe('insert');
      expect(dropdown.priority).toBe(200);
    }
  });

  it('dropdown contains 11 button items', () => {
    const items = Table.config.addToolbarItems?.call(Table);
    const dropdown = items?.[0];
    if (dropdown?.type === 'dropdown') {
      expect(dropdown.items).toHaveLength(11);
  ...
  it('all items have unique names', () => {
    const items = Table.config.addToolbarItems?.call(Table);
    const dropdown = items?.[0];
    if (dropdown?.type === 'dropdown') {
      const names = dropdown.items.map(i => i.name);
  ...
  it('includes expected commands', () => {
    ...
    const dropdown = items?.[0];
    if (dropdown?.type === 'dropdown') {
      const commands = dropdown.items.map(i => i.command);
      expect(commands).toContain('insertTable');
      expect(commands).toContain('deleteTable');
      expect(commands).toContain('addRowBefore');
      expect(commands).toContain('addRowAfter');
      expect(commands).toContain('deleteRow');
      expect(commands).toContain('addColumnBefore');
      expect(commands).toContain('addColumnAfter');
      expect(commands).toContain('deleteColumn');
      expect(commands).toContain('toggleHeaderRow');
      expect(commands).toContain('toggleHeaderColumn');
      expect(commands).toContain('toggleHeaderCell');
    }
  });
});
