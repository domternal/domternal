...
const staticMessages = emojiMessages;

describe('German emoji language pack', () => {
  it('covers exactly the owned message and searchable catalogs with frozen values', () => {
    const definitions = Object.values(emojiMessages);
  ...
  it('requires explicit opt in and leaves other editor services in English', () => {
    ...
    expect(german.t(emojiMessages.insert)).toBe('Emoji einfügen');
    expect(english.resolve(emojiMessages.insert)).toEqual({
      text: 'Insert Emoji', language: 'en', source: 'default',
    });
    german.set({ locale: 'de' });
    expect(german.resolve(emojiMessages.insert)).toEqual({
      text: 'Insert Emoji', language: 'en', source: 'default',
    });
    english.destroy();
    german.destroy();
